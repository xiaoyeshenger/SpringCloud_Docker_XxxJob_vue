...
@Setter
@Component
public class SignUpExcelListener extends AnalysisEventListener<SignUpExcelVo> {
    ...
    private List<SignUpExcelVo> signUpExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public SignUpExcelListener() {}

    public SignUpExcelListener(SignUpService signUpService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(SignUpExcelVo signUpExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(signUpExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        signUpExcelVoList.add(signUpExcelVo);
    }

    //2.参数校验
    private void validate(SignUpExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(SignUpExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(SignUpExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<SignUpExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<SignUpExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        signUpExcelVoList.forEach(e -> {
            SignUpReqDto signUpReqDto = VoPoConverterUtil.copyProperties(e, SignUpReqDto.class);
            signUpService.insertOrUpdateSignUp(signUpReqDto);
        });
    }
}