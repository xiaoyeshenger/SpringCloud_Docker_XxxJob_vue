...
public class ProjectExcelListener extends AnalysisEventListener<ProjectExcelVo> {
    ...
    private List<ProjectExcelVo> projectExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public ProjectExcelListener() {}

    public ProjectExcelListener(ProjectService projectService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    public void invoke(ProjectExcelVo projectExcelVo, AnalysisContext analysisContext) {
        ...
        validate(projectExcelVo,currentRowNum);
    ...
    //2.参数校验
    private void validate(ProjectExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(ProjectExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(ProjectExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<ProjectExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<ProjectExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        projectExcelVoList.forEach(e -> {
            ProjectReqDto projectReqDto = VoPoConverterUtil.copyProperties(e, ProjectReqDto.class);
            projectService.insertOrUpdateProject(projectReqDto);
        });
    }
}