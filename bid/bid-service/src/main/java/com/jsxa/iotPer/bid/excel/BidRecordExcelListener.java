...
@Setter
@Component
public class BidRecordExcelListener extends AnalysisEventListener<BidRecordExcelVo> {
    ...
    private List<BidRecordExcelVo> bidRecordExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public BidRecordExcelListener() {}

    public BidRecordExcelListener(BidRecordService bidRecordService, Validator validator, FieldDupValidator fieldDupValidator) {
    ...
    @Override
    public void invoke(BidRecordExcelVo bidRecordExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(bidRecordExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        bidRecordExcelVoList.add(bidRecordExcelVo);
    }

    //2.参数校验
    private void validate(BidRecordExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(BidRecordExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(BidRecordExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<BidRecordExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<BidRecordExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        bidRecordExcelVoList.forEach(e -> {
            BidRecordReqDto bidRecordReqDto = VoPoConverterUtil.copyProperties(e, BidRecordReqDto.class);
            bidRecordService.insertOrUpdateBidRecord(bidRecordReqDto);
        });
    }
}